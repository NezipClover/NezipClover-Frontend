...
<template>
  ...
  ></v-select>
      ...
      <VCol cols="12" v-if="this.select.val==3">
        <VTextField ref="r3"
...
export default {
    ...
    computed: {
      ...
      items () {
        return this.entries.map(entry => {
          const Description = entry.dongName.length > this.descriptionLimit
    ...
    watch: {
      search (val) {
        ...
        // Lazily load input items
         axios.get('http://localhost:8080/house/list')
          .then(res => {
            console.log(res);
            const { data } = res
    ...
    async checkHandler() {
      ...
      err && !this.onsale.dealAmount&&!this.onsale.rentMoney && ((msg = '거래금액을 입력해주세요'), (err = false), this.$refs.content.focus());
      err && !this.onsale.dealer_email && ((msg = '중개사 이메일을 입력해주세요'), (err = false), this.$refs.content.focus());
      if (!err) alert(msg);
